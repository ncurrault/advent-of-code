...
# for i in range(10):
#     state = get_valley_after(init_valley_state, i)
#     print(i)
#     print_blizzards(state)
#     print()
...
while True:
    ...
    if current.row == height - 1 and current.col == width - 1:
        if phase == Phase.THERE:
            to_explore = PriorityQueue()
            current = Node(row=height, col=width - 1, num_mins=next_min)
            phase = Phase.BACK
        elif phase == Phase.THERE_AGAIN:
            print(current.num_mins + 1)  # need to actually exit
            break
    elif phase == Phase.BACK and current.row == 0 and current.col == 0:
        phase = Phase.THERE_AGAIN
        to_explore = PriorityQueue()
        current = Node(row=-1, col=0, num_mins=next_min)
