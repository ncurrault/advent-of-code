...
# A* search, constrained based on <= +1 elevation criterion
elevation_grid = []
origin = None
destination = None
...
to_explore.add_task((origin, 0), priority=manhattan_distance(origin, destination))
explored = set()
while True:
    ...
    for new_square in valid_steps(*square):
        if new_square not in explored:
            to_explore.add_task(
                ...
                priority=manhattan_distance(new_square, destination) + steps_so_far,
            )
